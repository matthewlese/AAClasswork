...
def first_index(arr, &blc)
  arr.each.with_index { |el, i| return i if blc.call(el) }
  nil
end
...
def multi_map(arr, n=1, &blc)
  new_arr = []
  arr.each do |ele|
    n.times { ele = blc.call(ele) }
    new_arr << ele
  end
  new_arr
end
...
def proctition(arr, &blc)
  trues = []
  falses = []
  arr.each do |ele|
    trues << ele if blc.call(ele)
    falses << ele if !blc.call(ele)
  end
  trues + falses
end
