...
def hash_mapped(hash, proc, &bloc)
  new_hash = Hash.new()
  hash.keys.each do |key|
    new_hash[bloc.call(key)] = proc.call(hash[key])
  end
  new_hash
end
...
def uncompress(str)
  ...
  letters.each_char.with_index do |letter, i|
    new_str += letter * nums[i].to_i
  end
  p new_str
end
    ...
    if word.length < 3
      ...
      if "aeiou".include?(word[0].downcase)
        ...
        word.each_char.with_index do |char, i|
          befores += char if !"aeiou".include?(char)
          if "aeiou".include?(char)
            afters += word[i..-1]
            break
          end
        end
        new_sent << (afters + befores + "ay")
      end
    end
  end
  new_sent.join(" ")
  ...
  sent_arr.each do |word|
    if word.length < 3
      ...
      if "aeiou".include?(word[-1])
        ...
        new_arr << (word + appendage.reverse)
      end
    end
  end
  new_arr.join(" ")
...
def alternating_vowel(sent)
  ...
  sent_arr.each.with_index do |word, idx|
    new_word = ""
    if idx == 0 || idx.even? #deal with first, third, etc. by removing its first vowel
      word.each_char.with_index do |char, i|
        if "aeiou".include?(char)
    ...
    else #deal with second, fourth, etc. by removing its last vowel
      word.reverse.each_char.with_index do |char, i|
        if "aeiou".include?(char)
          new_word = word.reverse[0...i] + word.reverse[i + 1..-1]
