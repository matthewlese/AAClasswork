...
def my_rotate!(array, amt)
    if amt > 0
        amt.times do
            ele = array.shift #removes the first element of the array and returns it (try .push)
            array << ele #add to the end of the array
        end
    else
        (-amt).times do
            ele = array.pop #removes and returns the last element of the array
            array.unshift(ele) #add to the beginning of the array
        end
    end
    array
end
